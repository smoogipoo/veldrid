namespace Veldrid
{
    ...
    public abstract class ResourceFactory
    {
        ...
        {
            return CreateGraphicsPipeline(ref description);
        }
        ...
        public Pipeline CreateGraphicsPipeline(ref GraphicsPipelineDescription description)
        {
            ...
            if (!Features.IndependentBlend)
            {
                ...
                {
                    var attachmentState = description.BlendState.AttachmentStates[0];
            ...
            foreach (var layoutDesc in description.ShaderSet.VertexLayouts)
            {
                bool hasExplicitLayout = false;
                uint minOffset = 0;

                foreach (var elementDesc in layoutDesc.Elements)
                {
        ...
        {
            return CreateComputePipeline(ref description);
        }
        ...
        {
            return CreateFramebuffer(ref description);
        }
        ...
        {
            return CreateTexture(ref description);
        }
        ...
        public Texture CreateTexture(ref TextureDescription description)
        {
            ...
            if (description.Type == TextureType.Texture1D && !Features.Texture1D) throw new VeldridException("1D Textures are not supported by this device.");
        ...
        {
            return CreateTextureView(ref description);
        }
        ...
        public TextureView CreateTextureView(ref TextureViewDescription description)
        {
            ...
            {
                throw new VeldridException(
                    "To create a TextureView, the target texture must have either Sampled, Storage, DepthStencil, or RenderTarget usage flags.");
            }
        ...
        {
            return CreateBuffer(ref description);
        }
        ...
        public DeviceBuffer CreateBuffer(ref BufferDescription description)
        {
            ...
            {
                if (!Features.StructuredBuffer) throw new VeldridException("GraphicsDevice does not support structured buffers.");

                if (description.StructureByteStride == 0) throw new VeldridException("Structured Buffer objects must have a non-zero StructureByteStride.");
            ...
            else if (description.StructureByteStride != 0) throw new VeldridException("Non-structured Buffers must have a StructureByteStride of zero.");

            if ((usage & BufferUsage.Staging) != 0 && usage != BufferUsage.Staging) throw new VeldridException("Buffers with Staging Usage must not specify any other Usage flags.");

            if ((usage & BufferUsage.UniformBuffer) != 0 && description.SizeInBytes % 16 != 0) throw new VeldridException("Uniform buffer size must be a multiple of 16 bytes.");
#endif
        ...
        {
            return CreateSampler(ref description);
        }
        ...
        {
#if VALIDATE_USAGE
            if (!Features.ComputeShader && description.Stage == ShaderStages.Compute) throw new VeldridException("GraphicsDevice does not support Compute Shaders.");

            if (!Features.GeometryShader && description.Stage == ShaderStages.Geometry) throw new VeldridException("GraphicsDevice does not support Compute Shaders.");
        ...
        {
            return CreateCommandList(ref description);
        }
        ...
        {
            return CreateResourceSet(ref description);
        }
        ...
        {
            return CreateSwapchain(ref description);
        }
